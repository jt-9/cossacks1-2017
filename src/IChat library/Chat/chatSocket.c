...
#define RESET(ptr)  {if (ptr) {gsifree(ptr); ptr = nullptr;} }
...
static void ciSocketThinkSend(ciSocket * sock)
{
	...
	while(sock->outputQueue.length > 0)
	{
		...
		// Can we send?
		///////////////
		ciSocketSelect(sock->sock, nullptr, &writeFlag, nullptr);
		if(!writeFlag)
			return;
...
static void ciSocketThinkRecv(ciSocket * sock)
{
	...
	while(CHATTrue)
	{
		// Check the read flag.
		///////////////////////
		ciSocketSelect(sock->sock, &readFlag, nullptr, nullptr);
		...
		////////////////////////
		rcode = recv(sock->sock, &sock->inputQueue.buffer[sock->inputQueue.length], RECV_LEN, 0);
		/*if(rcode == SOCKET_ERROR)
			return; //ERRCON
*/
		// Connection closed?
				  ...
				  const char * buffer)
{
	...
	///////////////////////////////
	memcpy(&sock->outputQueue.buffer[sock->outputQueue.length], buffer, len);
...
{
	char *pTmpNick = nullptr, *pTmpUsername = nullptr, *pTmpHost = nullptr;
...
static CHATBool ciParseMessage(const char *sText, ciServerMessage * message)
{	
		...
		if(*p != '\0')
		{
			...
			if(*q != '\0')
			{
				...
				*r = '\0';
				...
				if(!ciParseMessage(p, &sock->lastMessage))
				{
					memset(&sock->lastMessage,0,sizeof(sock->lastMessage));
					return CHATFalse; //ERRCON
				}
