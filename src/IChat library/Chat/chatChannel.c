...
#define ASSERT_CHANNEL()   assert(channel != nullptr); assert(channel[0] != '\0'); assert(strlen(channel) < MAX_CHANNEL);
#define ASSERT_USER(user)  assert(user != nullptr); assert(user[0] != '\0'); assert(strlen(user) < MAX_NAME);
...
CHATBool ciInitChannels(ciConnection * connection)
{
	...
	if(connection->channelTable == nullptr)
		return CHATFalse;

	connection->enteringChannelList = ArrayNew(sizeof(ciChatChannel), 0, nullptr);
	if(connection->enteringChannelList == nullptr)
	{
...
CHATBool ciInChannel(CHAT chat, const char * channel)
{
	...
	if(ciGetChannel(connection, channel) == nullptr)
		return CHATFalse;
...
void ciSetChannelPassword(CHAT chat, const char * channel, const char * password)
{
	...
	if(chatChannel->password == nullptr)
		return; //ERRCON
...
void ciUserEnteredChannel(CHAT chat, const char * name, const char * channel, int mode, const char * user, const char * address)
{
	...
	// Check that we're in.
	///////////////////////
	assert(TableLookup(chatChannel->users, &chatUser) != nullptr);
}
...
static void ciUserChangeNickMap(void * elem, void * clientData)
{
	...
	if(user != nullptr)
	{
		...
		if(ciWasJoinCallbackCalled(data->chat, channel->name))  //PANTS - 03.01.00 - check if the join callback was called
		{
			// Add the callback.
			////////////////////
			if(channel->callbacks.userChangedNick != nullptr)
			{
...
void ciUserChangedMode(CHAT chat, const char * user, const char * channel, int mode, CHATBool enabled)
{
	...
	// Add the callback.
	////////////////////
	if(chatChannel->callbacks.userModeChanged != nullptr)
	{
...
CHATBool ciUserInChannel(CHAT chat, const char * channel, const char * user)
{
	...
	strcpy(chatUser.name, user);
	if(TableLookup(chatChannel->users, &chatUser) == nullptr)
		return CHATFalse;
...
static void ciClearAllUsersChannelMap(void * elem, void * clientData)
{
	...
	///////////////////////////////////////
	if(channel->callbacks.userListUpdated != nullptr)
	{
